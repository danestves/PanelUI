...
@layer theme {
  :root {
    @variant light { --color-primary: #6d28d9; --color-primary-foreground: #ffffff; }
    @variant dark  { --color-primary: #a78bfa; --color-primary-foreground: #1e1b4b; }
  }
}
```
...
```tsx
const [background, border] = useCSSVariable(['--color-background', '--color-border']);
```
