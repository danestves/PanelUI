...
A calendar of bins shaded by how much happened in each — one column per period, one row per bin inside it.

It answers "when was this busy" at a glance, which a line cannot: a year of daily numbers plotted as a series is a hairball, and the same numbers as a grid are a pattern.

It is the wrong shape for reading exact values. Use it to find where to look, and a [BarChart](/docs/charts/bar-chart) or the readout to say how much.
...
**Values:** `HeatmapChart`, `useHeatmapChart`, `buildHeatmapCalendar`

**Types:** `HeatmapChartProps`, `HeatmapCellsProps`, `HeatmapSeparatorProps`, `HeatmapXAxisProps`, `HeatmapYAxisProps`, `HeatmapTooltipProps`, `HeatmapLegendProps`, `HeatmapColumn`, `HeatmapBin`, `HeatmapCell`, `HeatmapLayout`
