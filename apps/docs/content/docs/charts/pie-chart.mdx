...
Every slice is a share of one total: the values are normalised against their sum, the angles come to a full turn, and a slice only means anything next to the others. That is the opposite claim to the [RingChart](/docs/charts/ring-chart) beside it, where each arc is measured against its own target and nothing has to add up.
