...
- **`LiveLineChart.Tip`** — The dot at the leading end and the current reading beside it. It rides the newest reading rather than the right-hand edge.
...
`paused` freezes the clock where it is. Readings still arrive and are still kept — it is the window that stops, so what is on screen holds still long enough to be talked about. Clearing it jumps the window back to the present.
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `badge` | `boolean` | `true` | Show the current reading beside the dot. |
