...
### Names between the wings

`labelPlacement="center"` moves the category names into a gutter down the middle — the shape a run of age bands is usually drawn in. The gutter is taken off the bars rather than off the edges, so both wings stay the same length as each other.
