...
<InstallTabs kind="dlx" packages="panelui-cli@latest init" />

<Callout>
  In an **empty** directory the same command does the other job instead: it starts a new app from
  a template, with the theme, the tab bar and the whole pipeline already wired — which is also
  what `npx create-panelui-app@latest` does. See [Templates](/docs/templates).
</Callout>
...
Aliases are directories inside the project: letters, numbers, dots, dashes and underscores,
separated by `/`, after an optional `@/`, `~/` or `./`. Anything else is refused. The alias is
written between the quotes of every import in every file copied in, so it has to be a path and
nothing else.
