...
An item is a wrapper, so what it holds is yours. A person is an avatar and a name; a colour is a swatch and a name; a slide is a thumbnail. Nothing about the component changes between them — only what is written inside the item.
  ...
  {palette.map((color) => (
    <SelectionMode.Item key={color.id} value={color.id}>
      <View className="flex-row items-center gap-3 py-1">
        <View style={{ backgroundColor: color.hex }} className="h-8 w-8 rounded-lg" />
