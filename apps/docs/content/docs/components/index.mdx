---
title: Components
description: Every component in PanelUI, by what it is for — 98 of them, each with its anatomy, props and variants.
---

All 98 components, grouped by the job they do. Every page covers the anatomy, every prop and every variant, read straight from the library's TypeScript so it cannot drift from the code.
