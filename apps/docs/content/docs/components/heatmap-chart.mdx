...
```tsx
<HeatmapChart data={week} layout="fill" rows={8} gap={4}>
  <HeatmapChart.Header title="Tickets opened" legend />
  <HeatmapChart.XAxis labels={['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']} />
  <HeatmapChart.YAxis labels={['00', '03', '06', '09', '12', '15', '18', '21']} tickFilter="all" width={24} />
  <HeatmapChart.Cells cornerRadius={3} />
  <HeatmapChart.Tooltip />
</HeatmapChart>
```
...
<HeatmapChart data={weeks.slice(-13)} layout="fill" weekStartDay={1} gap={4}>
  <HeatmapChart.Header title="Last 13 weeks" caption="The width divided between the columns" />
  <HeatmapChart.XAxis />
  <HeatmapChart.YAxis />
  <HeatmapChart.Cells cornerRadius={3} />
  <HeatmapChart.Tooltip />
...
Rows that are hours and columns that are weekdays. “When is this busy” is a question the year grid cannot answer, having already spent its rows on the days of the week — `rows` and a pair of `labels` turn the same grid on its side.

```tsx
<HeatmapChart data={week} layout="fill" rows={8} gap={4} onActiveCellChange={setActive}>
  <HeatmapChart.Header title="Tickets opened" value="9am – 6pm" legend />
