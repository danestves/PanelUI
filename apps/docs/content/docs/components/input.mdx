...
| `interactiveContent` | `boolean` | `false` | Let touches reach the content instead of falling through to the field. Off by default: an icon is decoration, and a tap anywhere on a field should put the caret in it rather than hitting a dead spot. Turn it on for content that does something — a clear button, a show-password toggle. |
...
Touches fall through to the field by default, so tapping an icon still puts the caret in the input. Pass `interactiveContent` for content that does something of its own — a clear button, a show-password toggle — and it takes its own presses and its own place in the accessibility tree.
