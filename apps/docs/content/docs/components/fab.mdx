...
`layout="menu"` unfolds one panel of rows out of the button instead of a column of buttons — the shape the platform's own menus take. Each row is its label with the glyph after it, and the panel springs out of the corner the trigger sits in, so it reads as the button opening rather than a sheet arriving.
...
Every variant takes the plain material, with its glyph in the foreground colour. A tint would only dim it: a primary colour on the glass turns it back into a fill, and a monochrome one greys it. `destructive` keeps a translucent red, because that colour carries meaning. Pressed, the material swells and brightens under the finger the way the platform's own glass controls do, in place of the usual press scale. On a `Fab.Group` the flag covers the trigger and every action.
...
**Types:** `FabProps`, `FabGroupProps`, `FabActionProps`, `FabPlacement`, `FabSize`, `FabVariant`, `FabGroupLayout`
