...
Every variant takes the plain material, with its glyph in the foreground colour. A tint would only dim it: a primary colour on the glass turns it back into a fill, and a monochrome one greys it. `destructive` keeps a translucent red, because that colour carries meaning. On a `Fab.Group` the flag covers the trigger and every action.
...
The list again, with the button in the platform material. Needs iOS 26 — anywhere else it is the filled one.
