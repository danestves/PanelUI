...
`primary` and `destructive` tint the material with their colour, so the action a screen leads with is still the one that stands out; `secondary` and `surface` take it plain. On a `Fab.Group` the flag covers the trigger and every action.
