...
Set `compact` and drop the grid, axis and crosshair: the line reaches the edges and the shape is the whole point. For the whole card around one — a headline number, a trend badge and the sparkline under it — reach for [KpiChart](/docs/components/kpi-chart) rather than assembling it each time.
