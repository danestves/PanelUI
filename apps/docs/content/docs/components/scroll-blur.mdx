...
`size` is how deep the band is, `intensity` how strong the blur gets at the very edge, and `layers` how many steps it is built from.

Raise `layers` with `size`: the same four steps spread over a deeper band are four visible bars.

```tsx
<ScrollBlur size={96} layers={6} intensity={60}>
  <ScrollView>{rows}</ScrollView>
</ScrollBlur>
```
