...
This is what makes a bento grid read as a bento grid rather than as a wall of stat cards. A sparkline pinned to the bottom, an oversized icon in a corner at low opacity, a photo: none of them are read for values, so none of them need axes.
...
This is the trade the grid makes, and it is the right way round for a bento: a grid of boxes that each grew to fit their own text is not a grid. But it does mean long text overflows rather than pushing the tile taller. Give it `numberOfLines`, drop the group to `size="sm"`, or give the tile another row.
