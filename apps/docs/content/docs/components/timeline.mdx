...
```tsx
import { Timeline, AppleIcon, GoogleIcon, View } from 'panelui-native';
```
...
`Timeline.Masthead` sits above the rail and says what the run is. It is one part rather than two `Text`s because the label and the name have to be typeset as one unit: set a step apart with an ordinary paragraph gap they read as a small heading followed by a bigger unrelated one.
...
`Timeline.Masthead` goes above the rail — outside the `Timeline`, because a horizontal one lays out columns and the masthead belongs to the whole run. Media, then a small label, then the name.
