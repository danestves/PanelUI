...
A column drops four points and scales down four percent as it leaves the reading edge, so the entry being read remains distinct without fading informative text. Dates, labels and descriptions stay fully opaque and retain the contrast of their theme tokens; contrast never depends on scroll position or reduced-motion preference.
