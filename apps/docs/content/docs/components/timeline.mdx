...
A flick lands on a column rather than between two, which is what `snap` is for; turn it off for free scrolling. Columns fade as they leave the reading edge, and the window is wide enough that the neighbours stay legible — being able to see what came before and after at once is most of the reason to draw a timeline at all. With the operating system set to reduce motion the fade is dropped and the rail simply scrolls.
