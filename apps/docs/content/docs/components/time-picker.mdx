...
One large readout over a scale you swipe. It is the layout that reads at arm’s length, so it is the one for a sheet — and the sheet is the presentation that gets a Done button, because a sheet’s outside is only the strip above it.
...
The same panel, centred over a dimmed screen. Worth it when the picker is the only decision on the screen and the trigger is somewhere awkward to anchor to.
...
<Frame>
  <Frame.Header>
    ...
    <Frame.Action>{formatTime(time)}</Frame.Action>
  </Frame.Header>
  <Frame.Panel>
    <Frame.Section>
      <ToggleButtonGroup value={mode} onValueChange={setMode}>
      ...
      <TimePicker
        ...
        onValueChange={mode === 'start' ? setStart : setEnd}
      />
    </Frame.Section>
  </Frame.Panel>
</Frame>
