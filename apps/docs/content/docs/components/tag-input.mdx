...
That is the whole distinction from [Combobox](/docs/components/combobox) in `multiple` mode. A Combobox picks from a set of options you supply, so it needs a list, a filter and a surface to float that list on. A tag field has no options and no list, so it carries none of that machinery and never opens a portal.
