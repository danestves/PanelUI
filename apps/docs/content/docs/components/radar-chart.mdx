...
```tsx
import { RadarChart, type RadarChartDatum, Frame } from 'panelui-native';
```
...
```tsx
<RadarChart data={scores} domain={[0, 100]}>
  <RadarChart.Grid />
  <RadarChart.Axis />
...
| `aspectRatio` | `number` | `1` | Width ÷ height. `1` is the square a radar wants; the rings are circular either way. |
