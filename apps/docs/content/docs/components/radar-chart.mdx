...
Unlike the other charts here a radar does not fill its container: filling a panel makes it as tall as the panel is wide, twice the height of the wide chart beside it for the same handful of numbers. `size` is the **diameter of the outermost ring** — not the box — and the view is that plus the room the labels need around it. It defaults to 180 and centres. Pass `size={undefined}` with an `aspectRatio` to fill the width instead.

```tsx
{/* Three sizes behind a set of tabs. */}
const sizes = { sm: 130, md: 180, lg: 230 };
