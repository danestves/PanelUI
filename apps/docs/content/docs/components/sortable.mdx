...
| `disabled` | `boolean` | `false` | Stop this row being picked up. The others still move past it, because a row that cannot be dragged is not the same as a row that cannot be displaced — a pinned row is a different feature, and pretending this one is it would mean silently refusing drops that look like they worked. |
| `activeClassName` | `string` | — | Extra classes for the row while it is being carried. |
