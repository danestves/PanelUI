...
- **`Map.Heatmap`** — Point density as a continuous field — the layer for *where*, once the points are too many to count.
...
Sessions by country, over a basemap dropped for the data. The shading and the cards above it come from the same tokens, so the map is part of the dashboard rather than an image pasted into it. Eight `Map.Label`s name the countries at `size="sm" tone="muted"` — they are the index to the shading, not the subject of the map.

<PreviewVideo
  src="/previews/map-analytics-map.mp4"
  poster="/previews/map-analytics-map-poster.jpg"
  alt="Map — Analytics map."
  width={720}
  height={1560}
/>
...
<Map blank bounds={[-11, 35, 31, 63]}>
  ...
  {Object.entries(capitals).map(([code, lngLat]) => (
    <Map.Marker key={code} lngLat={lngLat}>
      <Map.Label size="sm" tone="muted">{code}</Map.Label>
    </Map.Marker>
  ))}
</Map>
```
...
The small end of the range — a map as one element inside a card. `interactive={false}` because at this size a stray pan is an accident: the card scrolls, the map does not. Only the four heaviest countries are named, because a shaded map with nothing written on it asks the reader to recognise a country by its outline.
