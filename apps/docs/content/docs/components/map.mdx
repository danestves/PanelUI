...
A street map that is the whole screen, with the search field, the locate control and the place card sitting over it. On a map every point of chrome is a point of geography you cannot see, so nothing here takes a strip of its own. The card appears only once something is selected, and the pin you picked is the only one carrying a name — four labels at once is a map with the names of everything on it and the shape of nothing.
