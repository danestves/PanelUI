...
Every slice is a share of one total: the values are normalised against their sum, the angles come to a full turn, and a slice only means anything next to the others. That is the opposite claim to the [RingChart](/docs/components/ring-chart) beside it, where each arc is measured against its own target and nothing has to add up.
...
- **`PieChart.Header`** — The strip above the dial — what the chart is of, what it reads, and a key for the colours. The chart introducing itself, as distinct from the caption on the card around it.
...
- **`PieChart.Legend`** — A swatch, a name and a percentage per slice — pressable in the same way the slices are, and usually the easier target of the two. Prefer `Header legend` on a chart that has a header.
...
<PieChart data={sources} size={208} innerRadius={0.58} padAngle={3} minAngle={6}>
  <PieChart.Header title="Sessions" legend />
...
<PieChart data={storage} size={208} innerRadius={0.66} startAngle={-135} endAngle={135} padAngle={2}>
  <PieChart.Header title="In use" legend />
...
```tsx
<PieChart data={spend} size={208} activeIndex={active} onActiveIndexChange={setActive}>
  <PieChart.Header value={money(slice ? slice.value : total)} caption={caption} legend />
  <PieChart.Slices />
</PieChart>
```
...
| `legend` | `boolean` | `false` | Draw a swatch and a name per slice along the trailing edge. Prefer this to `PieChart.Legend` on a chart that has a header: that legend hangs off the bottom of the square, where it overlaps whatever is under the chart. |
...
Anything measured against its own target rather than against the others is a [RingChart](/docs/components/ring-chart), not a pie. Anything over time is a line or an area.
...
A slice can be pressed, but a slice worth two percent is not a touch target on a chart 208 points across. `PieChart.Legend` and `Header legend` put the same selection on a row of text, which is reachable at any share — prefer them, and treat pressing the dial itself as the shortcut rather than as the way in.
