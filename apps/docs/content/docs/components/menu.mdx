...
The panel's surface is a layer rather than a background, so it can be replaced. `Menu.Background` fills the panel behind the rows and inside its corner radius — pass one with a tint, a gradient, an image or a blur view inside it and it stands in for the default. Anything you put there is clipped to the panel and never scrolls with the rows.
