...
Both the fill **and the corner radius** follow the active theme: the variants resolve to themed surface tokens, and `rounded-3xl` resolves through the per-theme radius scale. The same Surface is a 24px corner under Panel and a 32px one under Moon, without knowing anything about either.
