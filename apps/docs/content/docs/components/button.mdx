...
`glass` draws a `native` button in the material iOS 26 uses for its own floating controls. `primary` and `destructive` take the prominent, tinted variant; every other variant takes the plain one. It needs `native` — on its own it does nothing — and on an older iOS, on Android, or without `@expo/ui` the button keeps its ordinary platform style rather than failing.
...
A `native` button at `size="icon"` is also drawn round rather than in the platform's default capsule — a lone glyph in a capsule reads as a text button somebody forgot to label.
