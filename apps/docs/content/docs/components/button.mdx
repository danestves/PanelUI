...
`primary` and `destructive` take the prominent variant, which keeps the accent tint a filled button is supposed to have; every other variant takes the plain one. A `native` icon button is also drawn round rather than in the platform's default capsule — a lone glyph in a capsule reads as a text button somebody forgot to label — and padded rather than sized, since a platform button style draws its background around the label and its padding, not around the frame.
