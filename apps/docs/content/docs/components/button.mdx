...
The material is drawn in the button's own shape: a circle at `size="icon"`, a capsule otherwise. `primary` and `destructive` tint it with the theme's accent; every other variant leaves it untinted.
