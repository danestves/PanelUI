...
| `thresholds` | `MeterThreshold[]` | — | Points on the scale where the colour changes, each `{ from, color }`. The highest one the reading has reached wins, so the order you list them in does not matter; below all of them the `color` prop applies. This is the difference between a meter and a bar: the colour is a judgement about the reading. Which direction is bad is yours to say — thresholds climbing to `destructive` suit a disk filling up, thresholds falling to it suit a battery running down. |
