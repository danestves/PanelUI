...
The surrounding `Frame` is the component’s own. Turn it off for a questionnaire inside something that already draws a boundary, and the header, question and footer stack bare.
...
<Card>
  <Card.Content>
    <Questionnaire frame={false}>
      ...
      <Questionnaire.Footer className="px-0 pb-0">
        <Questionnaire.Spacer />
        <Questionnaire.Submit />
      </Questionnaire.Footer>
    </Questionnaire>
