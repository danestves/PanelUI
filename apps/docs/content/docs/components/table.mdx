...
`Table.Frame` is the one place that rule gets easier rather than harder. The headings end up outside the card, but they are still the same `Table.Header` you would have written inside it — so the columns are declared once and the lift keeps them measuring against the same padding as the rows.
...
Column headings are not data. `Table.Frame` puts them on the tray above the card, where they read as the label for the block, and leaves the card holding nothing but rows. Give it the whole table — it takes the `Table.Header` out itself, so the columns are still declared once.
