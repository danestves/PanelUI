...
| `showPauseControl` | `boolean` | `true` | Show the built-in user pause/play control while motion is enabled. |
...
Motion includes a visible 48-point Pause/Play control by default. Its labels can be localized with `pauseLabel` and `playLabel`; use `onPlayingChange` to observe the user choice. Set `showPauseControl={false}` only when an equivalent visible control exists elsewhere. `playing={false}` remains the externally owned way to hold the content still and suppresses the built-in control.
