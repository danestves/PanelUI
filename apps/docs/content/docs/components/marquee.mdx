...
| --- | --- | --- | --- |
| `className` | `string` | — |  |
| `speed` | `number` | `DEFAULT_SPEED` | Travel speed in points per second. The cycle time follows from it and the measured content, so longer content takes proportionally longer rather than moving faster. |
