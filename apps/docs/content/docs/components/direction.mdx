...
```tsx
import { Direction, useDirection, useDirectionSign } from 'panelui-native';
```
...
- **Chevrons and arrows.** Yoga moves a chevron to the other end of its row but cannot turn the glyph around, which leaves an RTL list row pointing back at its own text. Directional glyphs mirror; a pencil or a magnifier does not, because those mean the same thing either way round.
