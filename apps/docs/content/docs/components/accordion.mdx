...
Collapsible sections for FAQs, settings groups and anything that would otherwise need a long scroll. Expansion animates with a Reanimated layout transition, and the content unmounts when collapsed — or stays mounted and hidden, with `keepMounted`, when there is state inside it worth keeping.
