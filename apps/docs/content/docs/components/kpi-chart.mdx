...
```tsx
import { KpiChart, Text, Frame } from 'panelui-native';
```
...
The chart under the number, full width and filled.
...
<KpiChart colorIndex={1}>
  ...
  <KpiChart.Value>$317,904</KpiChart.Value>
  <KpiChart.Chart data={week} dataKey="v" height={88} />
</KpiChart>
```
...
Several metrics as one panel — across a row, and down a column.

```tsx
<KpiChart.Group orientation="vertical">
  {services.map((service) => (
    <KpiChart key={service.label} surface={false} goodDirection="down">
      <KpiChart.Content layout="inline">
        <KpiChart.Stat>
          <KpiChart.Title>{service.label}</KpiChart.Title>
          <KpiChart.Value>{service.value}</KpiChart.Value>
        </KpiChart.Stat>
        <KpiChart.Trend value={service.delta} variant="badge" />
      </KpiChart.Content>
    </KpiChart>
  ))}
</KpiChart.Group>
