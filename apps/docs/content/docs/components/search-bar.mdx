...
When `disabled`, the field, clear button, Cancel button, and submit boundary are all inert. Cancel remains visible when `cancel="always"`, but leaves the focus order and cannot clear, blur, or call `onCancel`.
