...
The tour measures on the frame after `onStepChange`, so a scroll issued there lands first and the spotlight arrives where the target does.

```tsx
const scroller = useRef<ScrollView>(null);
const offsets = { 0: 0, 1: 320, 2: 720 };

<ScrollView ref={scroller}>
  <Tour
    open={running}
    onOpenChange={setRunning}
    onStepChange={(order) => scroller.current?.scrollTo({ y: offsets[order], animated: false })}
  >
    {/* …steps… */}
  </Tour>
</ScrollView>
