...
```tsx
import { AnimatedBadge, Avatar, Button, Card, CalendarIcon, DownloadIcon, Item, ListChecksIcon, ReceiptIcon, RepeatIcon, SparklesIcon } from 'panelui-native';
```
...
<View className="flex-row flex-wrap items-center gap-x-1.5 gap-y-2">
  {'Good morning,'.split(' ').map((word) => (
    <Text key={word} size="xl" muted>{word}</Text>
  ))}
  <Avatar size="sm" className="h-6 w-6" source={{ uri: photo }} fallback="AK" />
  <Text size="xl" weight="semibold">Alexey.</Text>
  <Text size="xl" muted>You</Text>
  <Text size="xl" muted>have</Text>
  ...
  <Text size="xl" muted>today.</Text>
</View>
```

### A summary that rewrites itself

The same shape as a written report, with every figure carried by a badge. Load a different period and each number rolls in place while the words around it stay put.
...
<Card>
  <Card.Content className="gap-5 p-5">
    ...
    <View className="flex-row flex-wrap items-center gap-x-1.5 gap-y-2">
      <Text size="lg" muted>You</Text>
      <Text size="lg" muted>have</Text>
      <Text size="lg" muted>had</Text>
      <AnimatedBadge size="sm" status="info" icon={<DownloadIcon size={12} />}>
        {report.downloads}
      </AnimatedBadge>
      <Text size="lg" muted>downloads</Text>
      <Text size="lg" muted>and</Text>
      <AnimatedBadge size="sm" status="success" icon={<ReceiptIcon size={12} />}>
        {report.revenue}
      </AnimatedBadge>
      <Text size="lg" muted>in</Text>
      <Text size="lg" muted>revenue.</Text>
    </View>
  </Card.Content>
</Card>
```
