...
**The front sizes the card.** The back is laid over it and given that box to fill, so a back with more in it than the front overflows rather than growing the card. Put a height on the root when the two faces differ; a card that changed height halfway through the turn would move everything under it.
...
The case the component exists for. The number, the holder and the expiry are on the front; the stripe and the security code are on the back, because that is where they are on the object.

Give the root a height. The back of a card carries less than the front and would otherwise be handed a box the front's content decided.
