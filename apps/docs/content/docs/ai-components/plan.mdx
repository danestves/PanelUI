...
The footer is the reason this is a card. A plan the reader has to agree to needs somewhere unambiguous for the agreement to live, separated from the steps by a rule. `Plan.Hint` states the shortcut next to the button it belongs to, rather than in a tooltip nobody opens.
