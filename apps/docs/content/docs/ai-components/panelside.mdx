...
- **`Panelside.Footer`** — The bar across the bottom of the panel. It floats over the list by default, with the list dissolving into the panel just above it — a solid bar would cut a strip out of the bottom of the scroller, and a transparent one would let rows collide with the controls.
- **`Panelside.Cta`** — The compose button. A pill rather than a rectangle, because it is the one control in the panel that is not a list row. Takes `native` to hand it to the platform.
...
| `edgeWidth` | `number` | `EDGE_WIDTH` | How wide the leading-edge strip that starts a swipe is. Default 48 — wider than the system's own edge gestures, because there is no bezel to feel for. Raise it on a screen with nothing else competing for a horizontal drag. |
