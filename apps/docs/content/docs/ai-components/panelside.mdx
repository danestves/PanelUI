...
```tsx
import { Panelside, AIInput, Avatar, BottomSheet, Button, MenuIcon, MessageCircleIcon, MicIcon, PackageIcon, PlusIcon, Select, StarIcon, View, usePanelside } from 'panelui-native';
```
...
They compose rather than exclude each other — a row taking its label from `label` and writing a trailing `Panelside.Action` as a child is the ordinary case.
...
The two forms compose: a row can take its label from the prop and still write a `Panelside.Action` as a child, which is what every example above does.
...
Panelside has no `native` prop and cannot have one — the platform toolkits ship a switch, a picker, a sheet and a button, and none of them is a pushing navigation panel. What goes native is what is inside it, and not all of it.

The composer is `AIInput`, drawn by us rather than handed over. A control the platform draws itself does not survive the scene being moved and scaled on a shared value, which is what the panel does to everything in it — so the native half of this screen is the controls that sit still: the picker, the sheet, and the panel's own compose button.

The composer's keyboard avoidance is off for the same reason. The panel is already moving the scene, and two things lifting one view is one lift too many.
