...
| `className` | `string` | — |  |
| `containerClassName` | `string` | — |  |
| `showClose` | `boolean` | `true` | Show the round dismiss button beside the field. On by default — it is the way out of the surface, and it belongs next to the thumb that is typing rather than in the corner furthest from it. |
