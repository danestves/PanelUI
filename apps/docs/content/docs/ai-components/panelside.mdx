...
Under `native` pass `nativeBackground` with it. A settings list is a column of rows on a surface, and the platform's translucent material with a moving screen behind it makes every one of those rows harder to read for nothing — the material is for a sheet you are meant to see past, and this is not one.
...
The account sheet is the platform's, painted solid with `nativeBackground`. A settings list is a column of rows on a surface, and the material with a moving screen behind it makes every one of those rows harder to read for nothing.
