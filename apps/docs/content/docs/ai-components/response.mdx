...
So this reads the markdown and renders it through the library's own parts — [Typography](/docs/components/typography) for prose, [CodeBlock](/docs/ai-components/code-block) for fences, [Table](/docs/components/table) for tables. Nothing here draws its own type or its own colours: an answer inside a message bubble should look like the app it is in, not like a document viewer someone embedded. It brings no new dependencies with it.
