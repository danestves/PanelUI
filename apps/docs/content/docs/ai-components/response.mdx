...
| `allowedLinkPrefixes` | `string[]` | `DEFAULT_LINK_PREFIXES` | Schemes a link is allowed to open. A model can write any URL it likes, and an answer is not a trusted document — so the default list is the four that cannot do anything but navigate. |
