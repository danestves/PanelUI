...
**A `native` toolbar is a taller row.** The platform frames its icon buttons at 44pt, so the row grows to hold them — a shorter one is a row its buttons hang out of, over the field above. The drawn controls are smaller and sit inside it either way.
