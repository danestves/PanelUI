...
`showStatus={false}` drops the sentence, for a grid where one status line covers every tile.
...
| --- | --- | --- | --- |
| `className` | `string` | — |  |
| `paused` | `boolean` | — | Stops the band and draws one still frame of the field. |
...
The field is around five hundred dots, and it costs twenty-two animated values a frame. Each column is one animated view holding its own dots, so the band's brightness is computed per column rather than per dot — the dots inside a column never move independently of it, and at this size nobody can tell.

The per-dot brightness underneath the band is fixed and derived from the dot's own coordinates, which is what stops the field reading as clean vertical stripes sweeping past.
