...
One status line over a row of tiles, rather than one under each. `showStatus={false}` drops the sentence and `resolution=""` drops the badge, which is what makes a tile small enough to sit beside others.
...
The box is the right shape from the moment the request goes in, so the reply under it never moves as the image lands. That is the whole job, and it is the one thing a card sitting alone on a page cannot show.
