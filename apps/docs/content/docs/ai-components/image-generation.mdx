...
| `animation` | `DotFieldAnimation` | `'drift'` | How the light behind the dots moves. `drift` wanders around the middle; `pulse` leaves the centre as a ring and fades; `scan` crosses the box as a band. All three cost the same, which is two numbers a frame however big the field is. |
