...
[Task](/docs/ai-components/task) is built for exactly this shape: a header that
says what the step is, a body for what it did, and four statuses that line up
with the four states one-for-one.
