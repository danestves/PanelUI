...
- `usage/<slug>.json` holds the parts a parser cannot infer, one component per file. **This is
  ...
  | Key | What it becomes |
  | --- | --- |
  | `preview` | A framed screenshot, the first thing in the body: `{ src, alt, width, height, caption? }`. Put the file in `public/previews/`, and give the real pixel dimensions — Next needs them for the aspect ratio |
  | `intro` | The paragraphs under the preview. The title and the `meta.json` summary are already above it, so do not open by restating either |
