...
- `PyramidChart.YAxis` — The category names, one per row. Reserves its own gutter, at the start or between the wings depending on `labelPlacement`.
...
| `labelPlacement` | `PyramidChartLabelPlacement` | `start` | Where the category names sit: down the left, or between the wings. |
