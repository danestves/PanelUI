...
/**
 * The charts, in the order the gallery shows them.
 *
 * Slugs rather than a second set of demos. The gallery renders each chart's
 * *first* version exactly as its own screen does — same card, same data, same
 * proportions — because a gallery that draws its own smaller version of every
 * chart is a second design to keep in step with the first, and the day they
...
/**
 * Every chart's catalogue entry, in gallery order.
 *
 * Filtered rather than asserted: a slug renamed above should drop a card off
 * the gallery, not crash the screen it was on.
 */
export const CHART_SHOWCASE: ComponentEntry[] = CHART_SLUGS.map(
  (slug) => COMPONENTS_BY_SLUG[slug]
